...
const PlayerHand = ({
  ...
  return (
    <div
      className={`p-4 rounded-lg ${isCurrentPlayer ? "bg-slate-200" : "bg-slate-100"}`}
    >
      ...
      <div className="flex gap-4 justify-center mb-6">
        {Object.entries(cardColors).map(([color, bgClass]) => (
          <div key={color} className="flex flex-col items-center">
            <div
              className={`w-16 h-24 ${bgClass} rounded-lg shadow-md flex items-center justify-center`}
            >
        ...
        <div>
          ...
          <div className="flex gap-1">
            {[...Array(8)].map((_, i) => (
              <div
                key={i}
                className="w-6 h-6 rounded-full bg-blue-400 border border-blue-600"
              ></div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex gap-1">
            {[...Array(3)].map((_, i) => (
              <div
                key={i}
                className="w-6 h-6 rounded-full bg-red-500 border border-red-700"
              ></div>
            ))}
          </div>
        </div>
